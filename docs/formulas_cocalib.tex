...
\subsubsection{Marginalization over $\vec{X_a}$}
To potentially raise the performance of the Gibbs sampler marginalization over $\vec{X_a}$ in the posterior of $a$ is investigated. 
From Bayes theorem it holds $p(\alpha, \beta) = p(\alpha|\beta) p(\beta)$ which here translates to
\begin{align}
    p(a, \vec{X_a}|b, \sigma_y, \vec{Y}) &= p(a|b, \vec{X_a}, \sigma_y, \vec{Y}) \cdot p(\vec{X_a} | \vec{\theta}, \sigma_y, \sigma_x, \vec{Y}, \vec{X_o})
\end{align}
Marginalizing by integration yields
\begin{align}
    p(a |b, \sigma_y, \vec{Y}) &= \int_{\mathbb{R}^N} p(a, \vec{X_a}|b, \sigma_y, \vec{Y}) d\vec{X_a} \\
    &= \int_{\mathbb{R}^N} p(a|b, \vec{X_a}, \sigma_y, \vec{Y}) \cdot p(\vec{X_a} | \vec{\theta}, \sigma_y, \sigma_x, \vec{Y}, \vec{X_o}) d\vec{X_a}
\end{align}
Replacing the expression with the values from above and performing the integration gives:
{\tiny
\begin{align}
    p(a |b, \sigma_y, \vec{Y}) \\
    &\propto \int_{\mathbb{R}^N} 
    \exp\left\{ 
        - \frac{1}{2\sigma_y^2} \sum_{i=1}^N (a^2 X_{ai}^2  + 2 a (b - Y_i) X_{ai} ) 
        - \frac{1}{2\sigma_a^2} ( a^2 - 2 a \mu_a )
    \right\} \cdot \exp\left\{ 
        - \frac{1}{2} (\vec{X_a} - \vec{M})^T \mat{V}^{-1} (\vec{X_a} - \vec{M}) 
    \right\} d\vec{X_a} \\
    
    &= \exp\left\{
        - \frac{1}{2\sigma_a^2} ( a^2 - 2 a \mu_a ) 
    \right\} \int_{\mathbb{R}^N} \exp\left\{ 
        - \frac{1}{2} (\vec{X_a}^T \mat{F_1} \vec{X_a} + \vec{X_a}^T \vec{F_2} + \vec{F_2}^T \vec{X_a}) 
    \right\} \exp\left\{ 
        - \frac{1}{2} (\vec{X_a} - \vec{M})^T \mat{V}^{-1} (\vec{X_a} - \vec{M}) 
    \right\} d\vec{X_a} \\
    
    &= \exp\{ \cdots \} \int_{\mathbb{R}^N} \exp\left\{ 
        - \frac{1}{2} (\vec{X_a}^T \underbrace{(\mat{F_1} + \mat{V}^{-1})}_{\mat{W}^{-1}} \vec{X_a} - \vec{X_a}^T \mat{W}^{-1} \underbrace{\mat{W} (\mat{V}^{-1} \vec{M} - \vec{F_2})}_{\vec{S}} - (\mat{V}^{-1} \vec{M} - \vec{F_2})^T \vec{X_a} + \vec{M}^T \mat{V}^{-1} \vec{M}) 
    \right\} d\vec{X_a} \\
    
    &= \exp\{ \cdots \} \int_{\mathbb{R}^N} \exp\left\{ 
        - \frac{1}{2} \bigg( (\vec{X_a} - \vec{S})^T \mat{W}^{-1} (\mat{X_a} - \vec{S}) + \vec{M}^T \mat{V}^{-1} \vec{M} - \vec{S}^T \mat{W}^{-1} \vec{S}) \bigg) 
    \right\} d\vec{X_a} \\
    
    &= \exp\{ \cdots \} \exp\left\{ 
        - \frac{1}{2} \bigg( (\vec{M}^T \mat{V}^{-1} \vec{M} - \vec{S}^T \mat{W}^{-1} \vec{S}) \bigg) 
    \right\} \int_{\mathbb{R}^N} 
    \exp\left\{ 
        - \frac{1}{2} (\vec{X_a} - \vec{S})^T \mat{W}^{-1} (\mat{X_a} - \vec{S}))
    \right\} d\vec{X_a} \\
    
    &\propto \exp\left\{
        - \frac{1}{2\sigma_a^2} ( a^2 - 2 a \mu_a )
    \right\} \exp\left\{ 
        - \frac{1}{2} (\vec{M}^T \mat{V}^{-1} \vec{M} - \vec{S}^T \mat{W}^{-1} \vec{S}) 
    \right\} 
    \frac{1}{\sqrt{\det \mat{W}}} \\
\end{align}
}%

If the definitions of $\mat{V}^{-1}$, $\mat{W}^{-1}$, $\vec{M}$ and $\vec{S}$ are further inserted, it can be seen that this contains terms in $a^2$, $a$, $a^{-1}$ and $a^{-2}$, which can not be represented as a normal distribution in $a$. 
To sample from this non-classic distribution involves optimization over expression given by numeric integration.
It needs to be investigated, whether this results in any performance benefits over not marginalizing the $\vec{X_a}$.
...
\begin{align}
    ...
    &\propto \exp\left\{ \sigma_y^2 \left( - \frac{1}{2\sigma_{\sigma}^2} \right) + \sigma_y \left( \frac{\mu_{\sigma}}{\sigma_{\sigma}^2}\right) + \frac{1}{\sigma_y^2} \left( - \frac{\tilde{A}}{2} \right) \right\} 
\end{align}
